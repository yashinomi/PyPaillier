...
class TestPaillierEncryption(unittest.TestCase):
    def test_enc_to_dec(self):
        ...
        message: int = 3
        self.assertEqual(message, keys.private_key.decrypt(keys.public_key.encrypt(message)))
        message = 10
        self.assertEqual(message, keys.private_key.decrypt(keys.public_key.encrypt(message)))
    ...
    def test_addition(self):
        ...
        self.assertEqual(
            (message1 + message2) % keys.public_key.n,
            keys.private_key.decrypt(keys.public_key.encrypt(message1) + keys.public_key.encrypt(message2))
            )
    ...
    def test_multiplication(self):
        ...
        self.assertEqual(
            (message * multiplier) % keys.public_key.n,
            keys.private_key.decrypt(keys.public_key.encrypt(message) * multiplier)
            )
